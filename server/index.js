...
async function requestClaudeRadarSetup(prompt) {
  if (!claudeKey) {
    const error = new Error("CLAUDE_KEY is missing on the server.");
...
app.post("/api/ai/radar-setup", asyncHandler(async (req, res) => {
  ...
  const text = await requestClaudeRadarSetup(prompt);
  res.json({ text, model: claudeModel });
}));
...
app.use((error, _req, res, _next) => {
  ...
  res.status(error?.statusCode || 500).json({ error: error instanceof Error ? error.message : "Erreur serveur" });
});
