...
const DEFAULT_SOURCES = [
  { id: "src-lemonde", name: "Le Monde - Numériques", url: "https://www.lemonde.fr/economie/rss_full.xml", category: "concurrence" },
...
async function writeStore(nextStore) {
  writeChain = writeChain.then(async () => {
    const finalStore = { ...nextStore, meta: { ...(nextStore.meta || {}), updatedAt: nowIso() } };
    const tmp = `${dataFile}.tmp`;
    await fs.writeFile(tmp, JSON.stringify(finalStore, null, 2), "utf8");
...
app.get("/api/health", async (_req, res) => {
  const store = await readStore();
  res.json({ ok: true, scanInProgress, sources: store.sources.length, articles: store.articles.length });
});
...
app.post("/api/sources", async (req, res) => {
  try {
    ...
    return res.status(400).json({ error: error instanceof Error ? error.message : "Impossible d'ajouter la source" });
  }
});
...
app.get("*", async (req, res, next) => {
  ...
  return res.status(503).send("Frontend non buildé. Exécutez d'abord npm run build.");
});
