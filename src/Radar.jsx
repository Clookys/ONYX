...
// ============================================================
const INIT_ENGINES = [
  {id:"en1",name:"Catégorie",icon:"\u25C9",color:"#a78bfa",description:"Classe par catégorie thématique.",mode:"select",extractRaw:false,values:[{v:"Réglementation",on:true},{v:"Politique",on:true},{v:"Innovation",on:true},{v:"AAP",on:true},{v:"Dispositif",on:true},{v:"Acquisition",on:true}],active:true,isDefault:true},
  ...
  {id:"en3",name:"Géographie",icon:"\u25C7",color:"#3b82f6",description:"Portée géographique.",mode:"scale",extractRaw:false,values:[{v:"Local",on:true},{v:"Régional",on:true},{v:"National",on:true},{v:"Européen",on:true},{v:"International",on:true}],active:true,isDefault:true},
  {id:"en4",name:"Sentiment",icon:"\u25C6",color:"#22c55e",description:"Ton général.",mode:"scale",extractRaw:false,values:[{v:"Positif",on:true},{v:"Neutre",on:true},{v:"Négatif",on:true}],active:true,isDefault:true},
  {id:"en5",name:"Secteur",icon:"\u2B21",color:"#f97316",description:"Secteurs d'activité.",mode:"select",extractRaw:false,values:[{v:"Industrie",on:true},{v:"Énergie",on:true},{v:"Numérique",on:true},{v:"Santé",on:true},{v:"Finance",on:true}],active:true,isDefault:true},
  {id:"en6",name:"Temporalité",icon:"\u23F1",color:"#ef4444",description:"Urgence et horizon.",mode:"scale",extractRaw:false,values:[{v:"Immédiat",on:true},{v:"Court terme",on:true},{v:"Moyen terme",on:true},{v:"Long terme",on:true}],active:true,isDefault:true},
];
...
const IC={
  ...
  scrape:(<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#6d9fff" strokeWidth="2"><path d="M12 2L2 7l10 5 10-5z"/><path d="M2 17l10 5 10-5"/></svg>),
  ...
  sparkle:(<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M12 2l2.4 7.2L22 12l-7.6 2.8L12 22l-2.4-7.2L2 12l7.6-2.8z"/></svg>),
  ...
  dot:(c)=>(<span style={{display:"inline-block",width:4,height:4,borderRadius:"50%",background:c,boxShadow:"none"}}/>),
  check:(<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#34d399" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>),
  loader:(<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="1s" repeatCount="indefinite"/></path></svg>),
...
// Cluster badge
function ClusterBadge({count}){return(<span style={{display:"inline-flex",alignItems:"center",gap:3,padding:"2px 7px",borderRadius:4,background:"rgba(109,159,255,0.08)",border:"1px solid #6d9fff22",color:"#6d9fff",fontSize:8,fontFamily:"'JetBrains Mono',monospace"}}>{IC.layers} {count} sources</span>);}

// ============================================================
// ALERTS PANEL
// ============================================================
function AlertsPanel({alerts,setAlerts,notifs,setNotifs,engines,onClose,onGoToArticle}){
  const[tab,setTab]=useState("notifs");const[nn,setNn]=useState("");const[nk,setNk]=useState("");const[ne,setNe]=useState("");const[nev,setNev]=useState("");const[nc,setNc]=useState("#f87171");
  ...
  return(
    <div style={{position:"fixed",top:0,right:0,bottom:0,width:380,maxWidth:"95vw",background:"#1e1e1e",borderLeft:"1px solid #ffffff08",boxShadow:"none",zIndex:900,display:"flex",flexDirection:"column"}}>
      <div style={{padding:"16px 18px 0",borderBottom:"1px solid #ffffff06"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>{IC.bell}<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:700,letterSpacing:2}}>ALERTES</span>{unread>0&&<span style={{padding:"1px 6px",borderRadius:8,background:"#f87171",color:"#fff",fontSize:8,fontWeight:700}}>{unread}</span>}</div>
          <button type="button" onClick={onClose} style={{background:"none",border:"none",color:"#ffffff25",cursor:"pointer",fontSize:14}}>x</button>
        </div>
        <div style={{display:"flex"}}>{[{id:"notifs",l:"Notifications"},{id:"rules",l:"Règles"},{id:"new",l:"Créer"}].map(t=>(<button key={t.id} type="button" onClick={()=>setTab(t.id)} style={{flex:1,padding:"8px 0",border:"none",borderBottom:`2px solid ${tab===t.id?"#f87171":"transparent"}`,background:"transparent",color:tab===t.id?"#f87171":"#ffffff22",fontSize:9,letterSpacing:1,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{t.l}</button>))}</div>
      </div>
      ...
      <div style={{flex:1,overflow:"auto",padding:"14px 18px"}}>
        {tab==="notifs"&&(<>
          ...
          {notifs.map(n=>{const al=alerts.find(a=>a.id===n.alertId);return(
            <div key={n.id} onClick={()=>{setNotifs(p=>p.map(x=>x.id===n.id?{...x,read:true}:x));onGoToArticle(n.articleId);onClose();}} style={{padding:"10px 12px",borderRadius:6,background:n.read?"transparent":"rgba(248,113,113,0.04)",border:`1px solid ${n.read?"#ffffff08":"#f8717115"}`,marginBottom:4,cursor:"pointer",borderLeft:`3px solid ${al?.color||"#f87171"}`}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}><span style={{fontSize:8,fontWeight:600,color:al?.color||"#f87171"}}>{al?.name}</span><span style={{flex:1}}/><span style={{fontSize:7,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace"}}>{n.time}</span>{!n.read&&<span style={{width:6,height:6,borderRadius:3,background:"#f87171"}}/>}</div>
              <div style={{fontSize:10,color:"#ffffff66",lineHeight:1.4}}>{n.text}</div>
            </div>);})}
        </>)}

        {tab==="rules"&&(<>
          {alerts.map(al=>(<div key={al.id} style={{padding:"10px 12px",borderRadius:6,background:"rgba(255,255,255,0.012)",border:`1px solid ${al.active?al.color+"15":"#ffffff08"}`,marginBottom:4,borderLeft:`3px solid ${al.color}`}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
              ...
              <button type="button" onClick={()=>setAlerts(p=>p.filter(a=>a.id!==al.id))} style={{background:"none",border:"none",color:"#ffffff10",cursor:"pointer",fontSize:9}} onMouseEnter={e=>{e.target.style.color="#f87171";}} onMouseLeave={e=>{e.target.style.color="#ffffff10";}}>x</button>
            </div>
            {al.keywords.length>0&&<div style={{fontSize:8,color:"#ffffff33",fontFamily:"'JetBrains Mono',monospace",marginBottom:2}}>Mots-clés : {al.keywords.join(", ")}</div>}
        ...
        {tab==="new"&&(<>
          ...
          <div style={{fontSize:8,letterSpacing:2,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace",marginBottom:4}}>MOTS-CLÉS (virgules)</div>
          ...
          <div style={{display:"flex",gap:4,marginBottom:8}}>
            <select value={ne} onChange={e=>{setNe(e.target.value);setNev("");}} style={{flex:1,padding:"7px",background:"#252525",border:"1px solid #ffffff08",borderRadius:5,color:"#ffffff88",fontSize:10,fontFamily:"'JetBrains Mono',monospace"}}><option value="">Aucune</option>{engines.map(en=>(<option key={en.id} value={en.id}>{en.name}</option>))}</select>
            {ne&&<select value={nev} onChange={e=>setNev(e.target.value)} style={{flex:1,padding:"7px",background:"#252525",border:"1px solid #ffffff08",borderRadius:5,color:"#ffffff88",fontSize:10,fontFamily:"'JetBrains Mono',monospace"}}><option value="">Toute valeur</option>{engines.find(e=>e.id===ne)?.values.filter(v=>v.on).map(v=>(<option key={v.v} value={v.v}>{v.v}</option>))}</select>}
          </div>
          <div style={{fontSize:8,letterSpacing:2,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace",marginBottom:4}}>COULEUR</div>
          <div style={{display:"flex",gap:3,marginBottom:12}}>{["#f87171","#ef4444","#ffb800","#22c55e","#3b82f6","#a78bfa","#6d9fff"].map(c=>(<button key={c} type="button" onClick={()=>setNc(c)} style={{width:20,height:20,borderRadius:4,background:c,border:nc===c?"2px solid #fff":"2px solid transparent",cursor:"pointer",opacity:nc===c?1:0.3}}/>))}</div>
          <button type="button" onClick={addAlert} style={{width:"100%",padding:10,borderRadius:6,border:"1px solid #f8717130",background:"rgba(248,113,113,0.06)",color:"#f87171",fontSize:10,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>CRÉER L'ALERTE</button>
        </>)}
      </div>
    </div>
  );
...
<h1 style="color:#1a1a2e;border-bottom:3px solid #a78bfa;padding-bottom:10px;">${title}</h1>
${intro?`<p style="color:#666;font-size:14px;line-height:1.6;">${intro}</p>`:""}
<p style="color:#999;font-size:12px;">${selected.length} articles sélectionnés</p>
${selected.map(a=>{
...
<div style="margin-top:20px;padding-top:15px;border-top:2px solid #a78bfa;font-size:11px;color:#999;text-align:center;">Généré par ONYX Radar</div>
</div>`;
  ...
  return(
    <div style={{position:"fixed",top:0,right:0,bottom:0,width:520,maxWidth:"95vw",background:"#1e1e1e",borderLeft:"1px solid #ffffff08",boxShadow:"none",zIndex:900,display:"flex",flexDirection:"column"}}>
      <div style={{padding:"16px 18px",borderBottom:"1px solid #ffffff06",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>{IC.mail}<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:700,letterSpacing:2}}>NEWSLETTER</span></div>
        <button type="button" onClick={onClose} style={{background:"none",border:"none",color:"#ffffff25",cursor:"pointer",fontSize:14}}>x</button>
      </div>
      ...
      <div style={{flex:1,overflow:"auto",padding:"14px 18px"}}>
        {!preview?(<>
          ...
          <div style={{maxHeight:200,overflow:"auto",marginBottom:14}}>
            {selected.length===0&&<div style={{padding:16,textAlign:"center",fontSize:10,color:"#ffffff15"}}>Sélectionnez des articles dans le feed pour les inclure</div>}
            {selected.map(a=>(<div key={a.id} style={{padding:"7px 10px",borderRadius:5,background:"rgba(255,255,255,0.012)",border:"1px solid #ffffff0a",marginBottom:2,display:"flex",alignItems:"center",gap:5,borderLeft:`3px solid ${a.sc}`}}>
          ...
          <button type="button" onClick={generate} disabled={generating||!selected.length} style={{width:"100%",padding:11,borderRadius:7,border:"1px solid #6d9fff30",background:generating?"transparent":"rgba(109,159,255,0.06)",color:"#6d9fff",fontSize:10,letterSpacing:2,cursor:generating?"wait":"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
            {generating?<>{IC.loader} GÉNÉRATION...</>:<>{IC.mail} GÉNÉRER LA NEWSLETTER</>}
          </button>
        </>):(
          <>
            <div style={{display:"flex",gap:5,marginBottom:12}}>
              ...
              <button type="button" onClick={()=>navigator.clipboard?.writeText(preview)} style={{flex:1,padding:8,borderRadius:5,border:"1px solid #6d9fff20",background:"rgba(109,159,255,0.04)",color:"#6d9fff",fontSize:9,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>COPIER HTML</button>
              <button type="button" style={{flex:1,padding:8,borderRadius:5,border:"1px solid #34d39920",background:"rgba(52,211,153,0.04)",color:"#34d399",fontSize:9,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>ENVOYER</button>
            </div>
            <div style={{fontSize:8,letterSpacing:2,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace",marginBottom:6}}>APERÇU</div>
...
function AISetupPanel({onClose,onApply,engines}) {
  ...
  const TYPES = [
    {id:"vt1",name:"Concurrentielle",icon:"\u2694",color:"#ffb800",desc:"Suivre vos concurrents et le marché",
      questions:[
        {id:"sector",label:"Votre secteur d'activité",placeholder:"Ex: SaaS B2B, Énergie renouvelable, BTP...",type:"text"},
        {id:"competitors",label:"Concurrents à surveiller (optionnel)",placeholder:"Ex: Datadog, Dynatrace, Splunk...",type:"text"},
        {id:"geo",label:"Zone géographique",type:"select",options:["France","Europe","International","Monde entier"]},
      ]},
    {id:"vt2",name:"Réglementaire",icon:"\u2696",color:"#a78bfa",desc:"Lois, décrets, directives, obligations",
      questions:[
        {id:"domain",label:"Domaine réglementaire",type:"multi",options:["Numérique / IA","Environnement","Travail / Social","Santé","Finance","Données / RGPD","Énergie","Industrie"]},
        {id:"geo",label:"Échelle",type:"select",options:["France uniquement","France + Europe","Europe uniquement","International"]},
      ]},
    {id:"vt3",name:"Appels à projets",icon:"\u2605",color:"#34d399",desc:"AAP, dispositifs, financements publics",
      questions:[
        {id:"sector",label:"Secteur visé",placeholder:"Ex: Transition écologique, Innovation numérique...",type:"text"},
        {id:"territory",label:"Territoire",placeholder:"Ex: Hauts-de-France, Auvergne-Rhône-Alpes...",type:"text"},
        {id:"funders",label:"Types de financeurs",type:"multi",options:["État / Ministères","Régions","Europe (FEDER, FSE, FTJ)","BPI / ADEME","ANR / Agences"]},
      ]},
    {id:"vt4",name:"Politique",icon:"\u26A0",color:"#f87171",desc:"Actualité politique et institutionnelle",
      questions:[
        {id:"focus",label:"Focus",type:"multi",options:["Gouvernement / Ministères","Parlement","Collectivités","Élections","Politique européenne"]},
        {id:"geo",label:"Échelle",type:"select",options:["France","France + Europe","International"]},
      ]},
    {id:"vt5",name:"Technologique",icon:"\u26A1",color:"#6d9fff",desc:"Innovations, R&D, tendances tech",
      questions:[
        {id:"techDomain",label:"Domaines tech",type:"multi",options:["Intelligence artificielle","Cybersécurité","Cloud / SaaS","Blockchain","IoT / Industrie 4.0","Green tech","Quantique","Robotique"]},
        {id:"depth",label:"Profondeur",type:"select",options:["Grand public (presse tech)","Professionnel (analyses)","Recherche (papers, brevets)"]},
      ]},
    {id:"vt6",name:"Sectorielle",icon:"\u2B21",color:"#f97316",desc:"Veille spécifique à votre industrie",
      questions:[
  ...
  const runAI = async () => {
    ...
    const prompt = `Tu es un expert en veille stratégique. L'utilisateur veut configurer une veille de type: ${selectedTypeNames}.

Contexte fourni:
${contextStr}

Génère une configuration de veille structurée en JSON UNIQUEMENT (pas de markdown, pas de backticks). Le JSON doit contenir:
{
  "folders": [
    {
      "name": "Nom du dossier",
      ...
      "suggestedEngines": ["Catégorie", "Géographie", "Sentiment"]
    }
  ]
}
...
- Privilégie des sources réelles et connues (sites institutionnels, médias reconnus, bases officielles)
- Adapte les sources au contexte géographique et sectoriel
- Suggère les analyses pertinentes parmi: Catégorie, Montant, Géographie, Sentiment, Secteur, Temporalité`;
    ...
    try {
      ...
      const colors = ["#ffb800","#a78bfa","#34d399","#f87171","#6d9fff","#f97316","#3b82f6","#22c55e"];
      ...
      const mock = selTypes.map((tid,i)=>{
        const t=TYPES.find(x=>x.id===tid);
        const colors=["#ffb800","#a78bfa","#34d399","#f87171","#6d9fff","#f97316"];
        return {
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:540,maxWidth:"94vw",maxHeight:"90vh",overflow:"auto",background:"#1e1e1e",border:"1px solid #a78bfa18",borderRadius:16,position:"relative"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"linear-gradient(90deg, #a78bfa, #6d9fff)",borderRadius:"16px 16px 0 0"}}/>
        <div style={{padding:"24px 22px 20px"}}>

          {/* Header */}
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
            {IC.sparkle}
            <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:700,letterSpacing:2}}>CONFIGURATEUR INTELLIGENT</span>
          </div>
          <div style={{fontSize:10,color:"#ffffff33",marginBottom:6}}>
            {step===1&&"Étape 1/3 — Choisissez vos types de veille"}
            {step===2&&"Étape 2/3 — Précisez votre contexte"}
            {step===3&&"Analyse en cours..."}
            {step===4&&"Étape 3/3 — Validez la configuration"}
          </div>
          {/* Progress bar */}
          <div style={{height:2,background:"#ffffff06",borderRadius:1,marginBottom:16}}>
            <div style={{height:2,background:"linear-gradient(90deg, #a78bfa, #6d9fff)",borderRadius:1,width:`${step===1?33:step===2?50:step===3?75:100}%`,transition:"width 0.5s"}}/>
          </div>
          ...
          {step===1&&(<>
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <button type="button" onClick={()=>setStep(2)} disabled={!selTypes.length} style={{flex:2,padding:10,borderRadius:7,border:`1px solid ${selTypes.length?"#a78bfa30":"#ffffff06"}`,background:selTypes.length?"rgba(167,139,250,0.06)":"transparent",color:selTypes.length?"#a78bfa":"#ffffff15",fontSize:9,letterSpacing:2,cursor:selTypes.length?"pointer":"default",fontFamily:"'JetBrains Mono',monospace"}}>SUIVANT ({selTypes.length})</button>
            </div>
          </>)}
          ...
          {step===2&&(<>
                  ...
                  {q.type==="select"&&(
                    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      {q.options.map(opt=>{
                        ...
                        return (<button key={opt} type="button" onClick={()=>updateCtx(q.id,opt)} style={{padding:"6px 11px",borderRadius:5,border:`1px solid ${sel?"#6d9fff25":"#ffffff06"}`,background:sel?"rgba(109,159,255,0.06)":"transparent",color:sel?"#6d9fff":"#ffffff30",fontSize:9,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{opt}</button>);
                      })}
                    </div>
                  )}
                  {q.type==="multi"&&(
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <button type="button" onClick={runAI} style={{flex:2,padding:10,borderRadius:7,border:"1px solid #a78bfa30",background:"rgba(167,139,250,0.06)",color:"#a78bfa",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>{IC.sparkle} GÉNÉRER MA VEILLE</button>
            </div>
          </>)}
          ...
          {step===3&&(
            <div style={{padding:"40px 20px",textAlign:"center"}}>
              ...
              <div style={{fontSize:12,color:"#a78bfa88",fontFamily:"'JetBrains Mono',monospace",marginBottom:6}}>Claude analyse votre contexte...</div>
          ...
          {step===4&&editResults&&(<>
                    ...
                    <div style={{padding:"0 12px 10px",display:"flex",flexDirection:"column",gap:2}}>
                      {folder.sources.map(s=>(
                        <div key={s.id} onClick={()=>tglSource(folder.id,s.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",borderRadius:4,background:s.enabled?"rgba(255,255,255,0.015)":"transparent",cursor:"pointer"}}>
                          ...
                          <span style={{fontSize:7,padding:"1px 4px",borderRadius:2,background:s.type==="rss"?"#ffb80008":"#6d9fff08",color:s.type==="rss"?"#ffb80055":"#6d9fff55",fontFamily:"'JetBrains Mono',monospace"}}>{s.type}</span>
                        </div>
                      ))}
                      {/* Suggested engines */}
            ...
            <div style={{padding:8,borderRadius:6,background:"rgba(52,211,153,0.03)",border:"1px solid #34d3990d",marginBottom:12,fontSize:9,color:"#34d39966",fontFamily:"'JetBrains Mono',monospace"}}>
              {editResults.filter(f=>f.enabled).length} dossier{editResults.filter(f=>f.enabled).length>1?"s":""} — {totalSrc} source{totalSrc>1?"s":""} au total
            </div>
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <button type="button" onClick={apply} disabled={!totalSrc} style={{flex:2,padding:10,borderRadius:7,border:"1px solid #34d39930",background:"rgba(52,211,153,0.05)",color:"#34d399",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{IC.check} APPLIQUER ({totalSrc} sources)</button>
            </div>
          </>)}
        </div>
      </div>
...
function EngineSettings({engine, onSave, onClose}) {
  ...
  const showRaw = engine.name==="Montant"||engine.name==="Géographie"||!engine.isDefault;

  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:460,maxWidth:"94vw",maxHeight:"85vh",overflow:"auto",background:"#1e1e1e",border:`1px solid ${engine.color}22`,borderRadius:16,position:"relative"}}>
        ...
        <div style={{padding:"22px 20px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
            ...
            <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:700,letterSpacing:2,color:engine.color}}>{engine.name}</span>
          </div>
          ...
          {showRaw&&(<div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:5}}>
              <button type="button" onClick={()=>setRaw(true)} style={{flex:1,padding:"10px",borderRadius:6,border:`1px solid ${raw?engine.color+"30":"#ffffff06"}`,background:raw?`${engine.color}08`:"transparent",cursor:"pointer",textAlign:"left"}}>
                <div style={{fontSize:10,fontWeight:700,color:raw?engine.color:"#ffffff40",marginBottom:2}}>Donnée brute</div>
                <div style={{fontSize:8,color:"#ffffff18"}}>{engine.name==="Montant"?"\"2.5M\u20AC\" tel quel":engine.name==="Géographie"?"\"Lyon\" tel quel":"Extraction directe"}</div>
              </button>
              <button type="button" onClick={()=>setRaw(false)} style={{flex:1,padding:"10px",borderRadius:6,border:`1px solid ${!raw?engine.color+"30":"#ffffff06"}`,background:!raw?`${engine.color}08`:"transparent",cursor:"pointer",textAlign:"left"}}>
                <div style={{fontSize:10,fontWeight:700,color:!raw?engine.color:"#ffffff40",marginBottom:2}}>{engine.name==="Montant"?"Fourchettes":engine.name==="Géographie"?"\u00C9chelle":"Valeurs"}</div>
                <div style={{fontSize:8,color:"#ffffff18"}}>Classe dans vos catégories</div>
              </button>
            </div>
          </div>)}
          ...
          {!raw&&(<div style={{marginBottom:14}}>
            ...
            {vals.map((val,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",borderRadius:5,background:val.on?`${engine.color}06`:"transparent",border:`1px solid ${val.on?engine.color+"12":"#ffffff08"}`,marginBottom:2}}>
                ...
                <button type="button" onClick={()=>rm(i)} style={{background:"none",border:"none",color:"#ffffff0d",cursor:"pointer",fontSize:9}} onMouseEnter={e=>{e.target.style.color="#f87171";}} onMouseLeave={e=>{e.target.style.color="#ffffff0d";}}>x</button>
              </div>
            ))}
          ...
          {raw&&(<div style={{padding:12,borderRadius:6,background:`${engine.color}06`,border:`1px solid ${engine.color}10`,marginBottom:14,fontSize:10,color:`${engine.color}88`,lineHeight:1.5}}>Mode brut : l'IA extraira la donnée directement depuis l'article.</div>)}
...
function AddSourceModal({onClose,folders,onAddSource}) {
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:440,maxWidth:"92vw",background:"#1e1e1e",border:"1px solid #ffffff10",borderRadius:16,position:"relative",maxHeight:"90vh",overflow:"auto"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"linear-gradient(90deg, #6d9fff, #a78bfa)",borderRadius:"16px 16px 0 0"}}/>
        <div style={{padding:"24px 22px 20px"}}>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,letterSpacing:4,color:"#6d9fff",marginBottom:14}}>+ SOURCE</div>
          <div style={{display:"flex",gap:5,marginBottom:10}}>
            <div style={{flex:1,position:"relative"}}><span style={{position:"absolute",left:7,top:"50%",transform:"translateY(-50%)"}}>{IC.globe}</span><input value={url} onChange={e=>{setUrl(e.target.value);setSr(null);}} placeholder="https://..." onKeyDown={e=>e.key==="Enter"&&scan()} style={{width:"100%",padding:"9px 8px 9px 24px",boxSizing:"border-box",background:"rgba(109,159,255,0.025)",border:"1px solid #ffffff08",borderRadius:6,color:"#fff",fontSize:11,outline:"none",fontFamily:"'JetBrains Mono',monospace"}}/></div>
            <button type="button" onClick={scan} disabled={scanning} style={{padding:"0 14px",borderRadius:6,border:"1px solid #6d9fff18",background:"rgba(109,159,255,0.04)",color:"#6d9fff",fontSize:9,cursor:scanning?"wait":"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",gap:3}}>{scanning?IC.loader:"SCAN"}</button>
          </div>
          {sr&&(<>
            <div style={{padding:10,borderRadius:6,marginBottom:10,background:sr.rss?"rgba(52,211,153,0.03)":"rgba(109,159,255,0.03)",border:`1px solid ${sr.rss?"#34d39915":"#6d9fff15"}`}}>
              ...
              <div style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:sr.rss?"#34d399":"#6d9fff"}}>{sr.rss?`RSS — ~${sr.n} articles`:`SCRAPING — ${sr.n} pages`}</div>
            </div>
            ...
            <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:6}}>
              ...
              <button type="button" onClick={()=>setUseNew(true)} style={{padding:"5px 10px",borderRadius:5,border:`1px solid ${useNew?"#6d9fff30":"#ffffff06"}`,background:useNew?"rgba(109,159,255,0.06)":"transparent",color:useNew?"#6d9fff":"#ffffff25",fontSize:9,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{IC.plus} Nouveau</button>
            </div>
            ...
            <div style={{padding:"10px 12px",borderRadius:6,background:`rgba(251,191,36,${withStatic?"0.05":"0.02"})`,border:`1px solid ${withStatic?"#ffb80025":"#ffffff06"}`,marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setWithStatic(!withStatic)}>
                ...
                <div>
                  <div style={{fontSize:10,fontWeight:600,color:withStatic?"#ffb800":"#ffffff44"}}>{"\u25A3"} Récupérer le contenu existant (Archives)</div>
                  <div style={{fontSize:8,color:"#ffffff20",marginTop:2}}>Le contenu déjà publié sera scrapé et envoyé dans l'onglet <span style={{color:"#ffb80055"}}>Archives</span> de la sidebar. Ce contenu est figé et exportable en Excel. Il ne sera pas dupliqué dans le dossier source dynamique.</div>
                </div>
              </div>
              {withStatic&&(<div style={{marginTop:8,paddingTop:8,borderTop:"1px solid #ffb80010"}}>
                <div style={{fontSize:8,color:"#ffb80055",fontFamily:"'JetBrains Mono',monospace",marginBottom:4}}>Profondeur de récupération :</div>
                ...
                <div style={{fontSize:7,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace"}}>Le contenu antérieur à {staticDays>=9999?"toujours":`${staticDays} jours`} ira dans les Archives. Les nouveaux contenus (veille dynamique) resteront dans le dossier source. Aucun doublon.</div>
              </div>)}
            </div>

            {/* Auto-archive option */}
            <div style={{padding:"8px 12px",borderRadius:6,background:"rgba(255,255,255,0.015)",border:`1px solid ${autoArchive?"#ffb80018":"#ffffff08"}`,marginBottom:12}}>
              <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setAutoArchive(!autoArchive)}>
                ...
                <div>
                  <div style={{fontSize:9,fontWeight:600,color:autoArchive?"#ffb800":"#ffffff33"}}>Archiver automatiquement après un délai</div>
                  <div style={{fontSize:7,color:"#ffffff18",marginTop:1}}>Les articles dynamiques seront déplacés vers les Archives après le délai choisi</div>
                </div>
              </div>
              {autoArchive&&(<div style={{marginTop:6,paddingLeft:26,display:"flex",gap:3}}>
                {[{d:7,l:"7j"},{d:14,l:"14j"},{d:30,l:"30j"},{d:60,l:"60j"},{d:90,l:"90j"}].map(o=>(<button key={o.d} type="button" onClick={()=>setArchiveDays(o.d)} style={{padding:"3px 7px",borderRadius:3,border:`1px solid ${archiveDays===o.d?"#ffb80025":"#ffffff0a"}`,background:archiveDays===o.d?"#ffb80008":"transparent",color:archiveDays===o.d?"#ffb800":"#ffffff1a",fontSize:7,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{o.l}</button>))}
              </div>)}
            </div>

            <div style={{display:"flex",gap:5}}>
              <button type="button" onClick={onClose} style={{flex:1,padding:10,borderRadius:6,border:"1px solid #ffffff06",background:"transparent",color:"#ffffff25",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>ANNULER</button>
              <button type="button" onClick={add} style={{flex:2,padding:10,borderRadius:6,border:"1px solid #6d9fff25",background:"rgba(109,159,255,0.05)",color:"#6d9fff",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>AJOUTER</button>
            </div>
          </>)}
        </div>
      </div>
...
function NewEngineModal({onClose, onAdd}) {
  ...
  const CS=["#6d9fff","#a78bfa","#f87171","#ffb800","#34d399","#3b82f6","#06b6d4","#ef4444","#f97316","#8b5cf6","#ec4899","#22c55e"];
  const add=()=>{
    if(!name.trim()||!prompt.trim()) return;
    const vList=raw?[]:valsStr.split(",").map(v=>v.trim()).filter(Boolean).map(v=>({v,on:true}));
    onAdd({id:`en${Date.now()}`,name:name.trim(),icon:"\u2726",color,description:"Analyse personnalisée",mode:"select",extractRaw:raw,values:vList,active:true,isDefault:false,customPrompt:prompt.trim()});
    onClose();
  };
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:420,maxWidth:"92vw",background:"#1e1e1e",border:"1px solid #ffffff12",borderRadius:16,position:"relative"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg, ${color}, #6d9fff)`,borderRadius:"16px 16px 0 0"}}/>
        <div style={{padding:"24px 22px 20px"}}>
          <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:700,letterSpacing:2,marginBottom:18}}>NOUVELLE ANALYSE</div>
          ...
          <div style={{display:"flex",gap:5,marginBottom:12}}>
            <button type="button" onClick={()=>setRaw(true)} style={{flex:1,padding:"10px",borderRadius:6,border:`1px solid ${raw?color+"35":"#ffffff08"}`,background:raw?`${color}0a`:"transparent",cursor:"pointer",textAlign:"left"}}><div style={{fontSize:10,fontWeight:700,color:raw?color:"#ffffff44",marginBottom:2}}>Donnée brute</div><div style={{fontSize:8,color:"#ffffff20"}}>Pas de valeurs prédéfinies</div></button>
            <button type="button" onClick={()=>setRaw(false)} style={{flex:1,padding:"10px",borderRadius:6,border:`1px solid ${!raw?color+"35":"#ffffff08"}`,background:!raw?`${color}0a`:"transparent",cursor:"pointer",textAlign:"left"}}><div style={{fontSize:10,fontWeight:700,color:!raw?color:"#ffffff44",marginBottom:2}}>Valeurs définies</div><div style={{fontSize:8,color:"#ffffff20"}}>L'IA choisit parmi votre liste</div></button>
          </div>
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            <button type="button" onClick={add} style={{flex:2,padding:11,borderRadius:8,border:`1px solid ${color}30`,background:`${color}0a`,color,fontSize:10,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>CRÉER</button>
          </div>
        </div>
      </div>
    </div>
...
// MAIN
// ============================================================
export default function ONYXRadar({ onBack, user, onLogout }){
  ...
  useEffect(()=>{const l=document.createElement("link");l.href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@300;400;500&display=swap";l.rel="stylesheet";document.head.appendChild(l);},[]);
  ...
  const addSource=(data)=>{
    if(data.folderName){
      const nf={id:data.folderId,name:data.folderName,color:["#6d9fff","#a78bfa","#ffb800","#34d399","#f87171"][Math.floor(Math.random()*5)],exp:true,sources:[{id:`s${Date.now()}`,name:data.name,type:data.type,ok:true,withStatic:data.withStatic}]};
      setSrcF(p=>[...p,nf]);
    } else {
  ...
  const applyAISetup=(data)=>{
    const nf={id:`sf${Date.now()}${Math.random()}`,name:data.folderName,color:data.color||["#6d9fff","#a78bfa","#ffb800","#34d399","#f87171"][Math.floor(Math.random()*5)],exp:true,sources:data.sources.map(s=>({id:s.id||`s${Date.now()}${Math.random()}`,name:s.name,type:s.type||"rss",ok:true,withStatic:false}))};
    setSrcF(p=>[...p,nf]);
  };
  ...
  const exportArchive=()=>{
    const rows=[["Source","Titre","Résumé","Date","URL"]];
  ...
  const AutoModal=()=>{
    ...
    const go=()=>{setRun(true);setTimeout(()=>{const r=sa.map(art=>{const na={};selE.forEach(eid=>{const en=engines.find(e=>e.id===eid);if(!en)return;if(en.extractRaw){na[eid]=["Donnée brute"];} else{const av=en.values.filter(v=>v.on);if(!av.length)return;na[eid]=av.sort(()=>0.5-Math.random()).slice(0,en.name==="Sentiment"?1:Math.floor(Math.random()*2)+1).map(p=>p.v);}});return{aid:art.id,na,sum:Object.values(na).flat().join(" \u2014 ")};});setRes(r);setRun(false);setDone(true);},2200);};
    const apply=()=>{handleAuto(res,mode);setShowAuto(false);};
    return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={()=>setShowAuto(false)}><div onClick={e=>e.stopPropagation()} style={{width:480,maxWidth:"94vw",maxHeight:"85vh",overflow:"auto",background:"#1e1e1e",border:"1px solid #ffffff12",borderRadius:16,position:"relative"}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"linear-gradient(90deg, #a78bfa, #6d9fff)",borderRadius:"16px 16px 0 0"}}/><div style={{padding:"22px 20px 18px"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14}}>{IC.sparkle}<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:700,letterSpacing:2}}>AUTO-ANALYSE</span><span style={{marginLeft:"auto",fontSize:9,color:"#ffffff20",fontFamily:"'JetBrains Mono',monospace"}}>{sa.length} art.</span></div>
      {!done?(<>
        {hasEx&&<div style={{display:"flex",gap:5,marginBottom:12}}>{[{m:"merge",l:"Fusionner",c:"#34d399"},{m:"overwrite",l:"Écraser",c:"#f87171"}].map(o=>(<button key={o.m} type="button" onClick={()=>setMode(o.m)} style={{flex:1,padding:8,borderRadius:6,border:`1px solid ${mode===o.m?o.c+"30":"#ffffff06"}`,background:mode===o.m?`${o.c}08`:"transparent",cursor:"pointer",fontSize:10,fontWeight:600,color:mode===o.m?o.c:"#ffffff40",textAlign:"center"}}>{o.l}</button>))}</div>}
        <div style={{marginBottom:12}}>{engines.filter(e=>e.active).map(en=>{const sel=selE.includes(en.id);return(<button key={en.id} type="button" onClick={()=>setSelE(p=>p.includes(en.id)?p.filter(i=>i!==en.id):[...p,en.id])} style={{display:"flex",alignItems:"center",gap:6,width:"100%",padding:"7px 9px",borderRadius:6,marginBottom:2,border:`1px solid ${sel?en.color+"22":"#ffffff08"}`,background:sel?`${en.color}06`:"transparent",cursor:"pointer"}}><span style={{width:14,height:14,borderRadius:3,border:`2px solid ${sel?en.color:"#ffffff0d"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{sel&&<span style={{width:6,height:6,borderRadius:2,background:en.color}}/>}</span><span style={{fontSize:12}}>{en.icon}</span><span style={{flex:1,fontSize:10,fontWeight:600,color:sel?en.color:"#ffffff38"}}>{en.name}</span></button>);})}</div>
        <div style={{display:"flex",gap:5}}><button type="button" onClick={()=>setShowAuto(false)} style={{flex:1,padding:9,borderRadius:6,border:"1px solid #ffffff06",background:"transparent",color:"#ffffff22",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>ANNULER</button><button type="button" onClick={go} disabled={run} style={{flex:2,padding:9,borderRadius:6,border:"1px solid #a78bfa30",background:run?"transparent":"rgba(167,139,250,0.06)",color:"#a78bfa",fontSize:9,letterSpacing:2,cursor:run?"wait":"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>{run?<>{IC.loader} ANALYSE...</>:<>{IC.sparkle} LANCER</>}</button></div>
      </>):(<>
        <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:10}}>{IC.check}<span style={{fontSize:11,fontWeight:600,color:"#34d399"}}>Terminé</span></div>
        <div style={{maxHeight:180,overflow:"auto",marginBottom:10}}>{res.map(r=>{const art=articles.find(a=>a.id===r.aid);return(<div key={r.aid} style={{padding:8,borderRadius:5,background:"rgba(255,255,255,0.01)",border:"1px solid #ffffff08",marginBottom:2}}><div style={{fontSize:9,color:"#ffffff55",marginBottom:3}}>{art?.title}</div><div style={{display:"flex",flexWrap:"wrap",gap:2}}>{Object.entries(r.na).map(([eid,vals])=>{const en=engines.find(e=>e.id===eid);return vals.map((v,i)=>(<ETag key={`${eid}${i}`} label={v} color={en?.color||"#fff"}/>));})}</div></div>);})}</div>
        <div style={{display:"flex",gap:5}}><button type="button" onClick={()=>setShowAuto(false)} style={{flex:1,padding:9,borderRadius:6,border:"1px solid #ffffff06",background:"transparent",color:"#ffffff22",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>ANNULER</button><button type="button" onClick={apply} style={{flex:2,padding:9,borderRadius:6,border:"1px solid #34d39930",background:"rgba(52,211,153,0.05)",color:"#34d399",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>APPLIQUER</button></div>
      </>)}
    </div></div></div>);
  };

  return(
    <div style={{display:"flex",height:"100vh",width:"100%",background:"#1a1a1a",fontFamily:"'DM Sans',sans-serif",color:"#fff",overflow:"hidden"}}>
      {/* SIDEBAR */}
      <aside style={{width:col?44:225,minWidth:col?44:225,borderRight:"1px solid #ffffff0a",background:"#171717",display:"flex",flexDirection:"column",transition:"0.25s",overflow:"hidden"}}>
        <div style={{padding:col?"10px 5px":"10px 10px",borderBottom:"1px solid #ffffff08",display:"flex",alignItems:"center",justifyContent:col?"center":"space-between"}}>
          {!col&&<div style={{display:"flex",alignItems:"center",gap:5}}>
            {onBack&&<button type="button" onClick={onBack} style={{background:"none",border:"1px solid #ffffff0d",borderRadius:4,color:"#ffffff33",cursor:"pointer",padding:"3px 6px",fontSize:8,fontFamily:"'JetBrains Mono',monospace"}} onMouseEnter={e=>{e.target.style.color="#6d9fff";e.target.style.borderColor="#6d9fff33";}} onMouseLeave={e=>{e.target.style.color="#ffffff33";e.target.style.borderColor="#ffffff0d";}}>←</button>}
            <div style={{width:20,height:20,borderRadius:4,background:"linear-gradient(135deg, #6d9fff, #a78bfa)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:900,fontFamily:"'DM Sans',sans-serif",boxShadow:"none"}}>O</div><div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:9,fontWeight:700,letterSpacing:3,background:"linear-gradient(90deg, #6d9fff, #a78bfa)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>ONYX</div><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:6,letterSpacing:2,color:"#ffffff15"}}>RADAR</div></div>
          </div>}
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            {!col&&onLogout&&<button type="button" onClick={onLogout} style={{background:"none",border:"none",color:"#ffffff10",cursor:"pointer",fontSize:7,fontFamily:"'JetBrains Mono',monospace"}} onMouseEnter={e=>{e.target.style.color="#f87171";}} onMouseLeave={e=>{e.target.style.color="#ffffff10";}}>logout</button>}
        ...
        {!col&&<div style={{flex:1,overflow:"auto",padding:"6px 8px"}}>
          ...
          {srcF.map((f,fi)=>(<div key={f.id}>
            <div style={{display:"flex",alignItems:"center",position:"relative"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",marginRight:2}}>
                <button type="button" onClick={()=>moveFolder(f.id,-1)} style={{background:"none",border:"none",color:fi===0?"#ffffff0a":"#ffffff15",cursor:fi===0?"default":"pointer",padding:0,fontSize:7,lineHeight:1}}>{"\u25B2"}</button>
                <button type="button" onClick={()=>moveFolder(f.id,1)} style={{background:"none",border:"none",color:fi===srcF.length-1?"#ffffff0a":"#ffffff15",cursor:fi===srcF.length-1?"default":"pointer",padding:0,fontSize:7,lineHeight:1}}>{"\u25BC"}</button>
              </div>
            ...
            {f.exp&&f.sources.map(s=>(<div key={s.id} style={{padding:"2px 5px 2px 32px",display:"flex",alignItems:"center",gap:3,fontSize:8,color:"#ffffff15"}}>
              ...
              {IC.dot(s.ok?"#34d399":"#f87171")}
              <button type="button" onClick={()=>setSrcF(p=>p.map(ff=>ff.id!==f.id?ff:{...ff,sources:ff.sources.filter(x=>x.id!==s.id)}))} style={{background:"none",border:"none",color:"#ffffff06",cursor:"pointer",padding:0,display:"flex",fontSize:7}} onMouseEnter={e=>{e.currentTarget.style.color="#f87171";}} onMouseLeave={e=>{e.currentTarget.style.color="#ffffff06";}}>x</button>
            </div>))}
          </div>))}
          ...
          {(()=>{
            ...
            return staticSources.length>0?(<>
              {/* All archives */}
              <button type="button" onClick={()=>setFilter({t:"static",id:"all"})} style={{width:"100%",padding:"4px 6px",borderRadius:4,border:"none",background:filter.t==="static"&&filter.id==="all"?"rgba(251,191,36,0.06)":"transparent",color:filter.t==="static"&&filter.id==="all"?"#ffb800":"#ffffff1a",fontSize:9,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:4,marginBottom:1}}>
              ...
              {staticSources.map(s=>(<button key={s.id+"_ar"} type="button" onClick={()=>setFilter({t:"static",id:s.id})} style={{width:"100%",padding:"3px 6px 3px 16px",borderRadius:3,border:"none",background:filter.t==="static"&&filter.id===s.id?"rgba(251,191,36,0.05)":"transparent",color:filter.t==="static"&&filter.id===s.id?"#ffb800":"#ffffff15",fontSize:8,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</span>
                <span style={{fontSize:6,color:"#ffb80020",fontFamily:"'JetBrains Mono',monospace"}}>figé</span>
              </button>))}
            </>):(<div style={{padding:"4px 6px",fontSize:8,color:"#ffffff0d",fontStyle:"italic"}}>Aucune source archivée</div>);
          })()}
          ...
          <div style={{fontSize:7,letterSpacing:3,color:"#ffffff0d",fontFamily:"'JetBrains Mono',monospace",padding:"10px 4px 3px",display:"flex",alignItems:"center",gap:4}}><span style={{flex:1,height:1,background:"#ffffff0a"}}/><span>ANALYSE</span><span style={{flex:1,height:1,background:"#ffffff0a"}}/></div>
          <button type="button" onClick={()=>setFilter({t:"allAn"})} style={{width:"100%",padding:"4px 6px",borderRadius:4,border:"none",background:filter.t==="allAn"?"rgba(167,139,250,0.06)":"transparent",color:filter.t==="allAn"?"#a78bfa":"#ffffff22",fontSize:10,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:5,marginBottom:2}}>{"\u25C8"} Toutes <span style={{marginLeft:"auto",fontSize:7,color:"#ffffff10",fontFamily:"'JetBrains Mono',monospace"}}>{allAn}</span></button>
          ...
          {/* NEWSLETTER */}
          <div style={{fontSize:7,letterSpacing:3,color:"#ffffff0d",fontFamily:"'JetBrains Mono',monospace",padding:"10px 4px 3px",display:"flex",alignItems:"center",gap:4}}><span style={{flex:1,height:1,background:"#ffffff0a"}}/><span>DIFFUSION</span><span style={{flex:1,height:1,background:"#ffffff0a"}}/></div>
          <button type="button" onClick={()=>setShowNews(true)} style={{width:"100%",padding:"4px 6px",borderRadius:4,border:"none",background:showNews?"rgba(109,159,255,0.06)":"transparent",color:showNews?"#6d9fff":"#ffffff22",fontSize:10,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:5}}>{IC.mail} Newsletter</button>

          <div style={{fontSize:7,letterSpacing:3,color:"#ffffff0d",fontFamily:"'JetBrains Mono',monospace",padding:"10px 4px 3px",display:"flex",alignItems:"center",gap:4}}><span style={{flex:1,height:1,background:"#ffffff0a"}}/><span>NOTES</span><span style={{flex:1,height:1,background:"#ffffff0a"}}/></div>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* HEADER */}
        <header style={{padding:"7px 14px",borderBottom:"1px solid #ffffff08",display:"flex",alignItems:"center",gap:6,background:"#17171780"}}>
          <div style={{position:"relative",flex:1,maxWidth:240}}><span style={{position:"absolute",left:7,top:"50%",transform:"translateY(-50%)"}}>{IC.search}</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Rechercher..." style={{width:"100%",padding:"6px 7px 6px 24px",boxSizing:"border-box",background:"rgba(255,255,255,0.01)",border:"1px solid #ffffff06",borderRadius:4,color:"#fff",fontSize:10,outline:"none"}}/></div>

          {/* Dedup toggle */}
          <button type="button" onClick={()=>setShowDupes(!showDupes)} style={{padding:"4px 9px",borderRadius:4,border:`1px solid ${showDupes?"#6d9fff22":"#ffffff06"}`,background:showDupes?"rgba(109,159,255,0.04)":"transparent",color:showDupes?"#6d9fff":"#ffffff22",fontSize:8,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",gap:3}}>{IC.layers} Dédup.</button>

          {/* Sort controls */}
          <div style={{display:"flex",gap:2,alignItems:"center"}}>
            <span style={{fontSize:7,color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace"}}>Tri:</span>
            {[{k:"date",l:"Date"},{k:"source",l:"Source"},{k:"status",l:"Statut"}].map(s=>(<button key={s.k} type="button" onClick={()=>setSortBy(s.k)} style={{padding:"3px 7px",borderRadius:3,border:`1px solid ${sortBy===s.k?"#ffffff18":"#ffffff08"}`,background:sortBy===s.k?"#ffffff08":"transparent",color:sortBy===s.k?"#ffffff66":"#ffffff18",fontSize:7,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{s.l}</button>))}
          </div>
          ...
          {/* Selection actions */}
          {selIds.length>0&&(<>
            <span style={{fontSize:8,color:"#6d9fff55",fontFamily:"'JetBrains Mono',monospace"}}>{selIds.length} sél.</span>
            <button type="button" onClick={()=>setShowAuto(true)} style={{padding:"4px 9px",borderRadius:4,border:"1px solid #a78bfa28",background:"rgba(167,139,250,0.04)",color:"#a78bfa",fontSize:8,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",gap:3}}>{IC.sparkle} Analyser</button>
            <button type="button" onClick={()=>setSelIds([])} style={{padding:"4px 5px",borderRadius:3,border:"1px solid #ffffff0a",background:"transparent",color:"#ffffff15",fontSize:8,cursor:"pointer"}}>x</button>
          </>)}

          {/* Export button - visible when viewing archives */}
          {filter.t==="static"&&(
            <button type="button" onClick={exportArchive} style={{padding:"4px 9px",borderRadius:4,border:"1px solid #ffb80020",background:"rgba(251,191,36,0.04)",color:"#ffb800",fontSize:8,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",gap:3}}>CSV</button>
          )}

          {/* Alerts bell */}
          <button type="button" onClick={()=>setShowAlerts(true)} style={{position:"relative",background:"none",border:"none",color:unreadNotifs>0?"#f87171":"#ffffff22",cursor:"pointer",padding:4,display:"flex"}}>
            {IC.bell}
            {unreadNotifs>0&&<span style={{position:"absolute",top:-2,right:-2,width:14,height:14,borderRadius:7,background:"#f87171",color:"#fff",fontSize:7,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>{unreadNotifs}</span>}
          </button>
        </header>
        ...
        <div style={{flex:1,overflow:"auto",padding:"10px 14px"}}>
          {selArt?(
            <div style={{height:"100%",overflow:"auto"}}>
              <button type="button" onClick={()=>setSelArtId(null)} style={{display:"flex",alignItems:"center",gap:4,padding:"4px 0 12px",background:"none",border:"none",color:"#6d9fff",cursor:"pointer",fontSize:10}}>{IC.back} Retour</button>
              ...
              {selArt.ai&&<div style={{padding:8,borderRadius:5,background:"rgba(167,139,250,0.03)",border:"1px solid #a78bfa0a",marginBottom:12,fontSize:9,color:"#a78bfa66",fontStyle:"italic",fontFamily:"'JetBrains Mono',monospace"}}>{"\u2728"} {selArt.ai}</div>}
              ...
              {Object.keys(selArt.an).length===0&&<div style={{fontSize:9,color:"#ffffff12",fontStyle:"italic",marginBottom:10}}>Pas encore analysé</div>}
              ...
              {selArt.cluster&&(<div style={{marginBottom:16}}><div style={{fontSize:8,letterSpacing:2,color:"#6d9fff44",fontFamily:"'JetBrains Mono',monospace",marginBottom:6}}>{IC.layers} ARTICLES SIMILAIRES</div>{articles.filter(a=>a.cluster===selArt.cluster&&a.id!==selArt.id).map(a=>(<div key={a.id} onClick={()=>setSelArtId(a.id)} style={{padding:"7px 10px",borderRadius:5,background:"rgba(109,159,255,0.02)",border:"1px solid #6d9fff10",marginBottom:3,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:8,color:a.sc,fontWeight:600}}>{a.sn}</span><span style={{fontSize:9,color:"#ffffff55",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.title}</span></div>))}</div>)}
              <a href={selArt.url} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:4,padding:"6px 10px",borderRadius:4,border:"1px solid #ffffff0a",background:"rgba(255,255,255,0.008)",color:"#ffffff44",fontSize:9,textDecoration:"none",fontFamily:"'JetBrains Mono',monospace"}}>{IC.link} Source</a>
            </div>
          ):(
            <>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                {feed.length===0&&(
                  <div style={{textAlign:"center",padding:"60px 20px"}}>
                    ...
                    <div style={{fontSize:14,color:"#555",fontWeight:500,marginBottom:6}}>Aucun article</div>
                    <div style={{fontSize:12,color:"#444",lineHeight:1.5}}>Ajoutez des sources avec <span style={{color:"#6d9fff"}}>+ Source</span> ou utilisez le <span style={{color:"#a78bfa"}}>✨ Configurateur IA</span> pour démarrer votre veille.</div>
                  </div>
                )}
                {feed.map(art=>{
                  const isSel=selIds.includes(art.id);const hasAn=Object.keys(art.an).length>0;
                  return(<div key={art.id} style={{padding:"10px 11px",borderRadius:6,border:`1px solid ${isSel?"#6d9fff1a":"#ffffff06"}`,borderLeft:`3px solid ${art.sc}`,background:isSel?"rgba(109,159,255,0.015)":"rgba(255,255,255,0.004)",transition:"0.1s"}} onMouseEnter={e=>{if(!isSel)e.currentTarget.style.background="rgba(255,255,255,0.01)";}} onMouseLeave={e=>{e.currentTarget.style.background=isSel?"rgba(109,159,255,0.015)":"rgba(255,255,255,0.004)";}}>
                    <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:4}}>
                      <button type="button" onClick={e=>{e.stopPropagation();tSel(art.id);}} style={{width:15,height:15,borderRadius:3,flexShrink:0,border:`1.5px solid ${isSel?"#6d9fff":"#ffffff0a"}`,background:isSel?"#6d9fff15":"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{isSel&&<span style={{color:"#6d9fff",fontSize:7,fontWeight:700}}>{"\u2713"}</span>}</button>
                      ...
                      {art.rg&&<span style={{padding:"0 4px",borderRadius:2,background:"#ffffff06",color:"#ffffff28",fontSize:7,fontFamily:"'JetBrains Mono',monospace"}}>{art.rg}</span>}
                      ...
                      {!hasAn&&<span style={{fontSize:7,padding:"1px 4px",borderRadius:2,background:"#ffffff08",color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace"}}>non analysé</span>}
                    ...
                    {art.ai&&<div style={{fontSize:8,color:"#a78bfa40",fontStyle:"italic",marginBottom:2,fontFamily:"'JetBrains Mono',monospace"}}>{"\u2728"} {art.ai}</div>}
      ...
      {ctxMenu&&(<>
        ...
        <div style={{position:"fixed",left:ctxMenu.x,top:ctxMenu.y,width:160,background:"#111122",border:"1px solid #ffffff12",borderRadius:8,boxShadow:"none",zIndex:999,padding:4}}>
          {[
            ...
            {l:"Supprimer",fn:()=>ctxDelete(ctxMenu.fid),c:"#f87171"},
          ].map((item,i)=>(
            item.isSub?(
              <div key={i} style={{padding:"4px 8px"}}>
                <div style={{fontSize:8,color:"#ffffff33",marginBottom:3}}>Couleur</div>
                <div style={{display:"flex",gap:2}}>
                  {["#6d9fff","#a78bfa","#f87171","#ffb800","#34d399","#3b82f6","#f97316"].map(c=>(
      ...
      <style>{`*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#ffffff08;border-radius:2px}a{color:inherit}button{font-family:inherit}`}</style>
    </div>
  );
}
