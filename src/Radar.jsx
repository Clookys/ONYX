...
// ============================================================
const INIT_ENGINES = [
  {id:"en1",name:"Catégorie",icon:"\u25C9",color:"#b026ff",description:"Classe par catégorie thématique.",mode:"select",extractRaw:false,values:[{v:"Réglementation",on:true},{v:"Politique",on:true},{v:"Innovation",on:true},{v:"AAP",on:true},{v:"Dispositif",on:true},{v:"Acquisition",on:true}],active:true,isDefault:true},
    ...
    <div style={{position:"fixed",top:0,right:0,bottom:0,width:380,maxWidth:"95vw",background:"#0b0b18",borderLeft:"1px solid #ffffff08",boxShadow:"-20px 0 60px rgba(0,0,0,0.6)",zIndex:900,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"14px 18px"}}>
        ...
        {tab==="new"&&(<>
          ...
          <button type="button" onClick={addAlert} style={{width:"100%",padding:10,borderRadius:6,border:"1px solid #ff003c30",background:"rgba(255,0,60,0.06)",color:"#ff003c",fontSize:10,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>CRÉER L'ALERTE</button>
        </>)}
      </div>
    </div>
  );
...
<div style="margin-top:20px;padding-top:15px;border-top:2px solid #b026ff;font-size:11px;color:#999;text-align:center;">Généré par ONYX Radar</div>
</div>`;
...
function AISetupPanel({onClose,onApply,engines}) {
  ...
  const TYPES = [
    ...
    {id:"vt2",name:"Réglementaire",icon:"\u2696",color:"#b026ff",desc:"Lois, décrets, directives, obligations",
      questions:[
    ...
    {id:"vt3",name:"Appels à projets",icon:"\u2605",color:"#00ff87",desc:"AAP, dispositifs, financements publics",
      questions:[
    ...
    {id:"vt4",name:"Politique",icon:"\u26A0",color:"#ff003c",desc:"Actualité politique et institutionnelle",
      questions:[
...
- Suggère les analyses pertinentes parmi: Catégorie, Montant, Géographie, Sentiment, Secteur, Temporalité`;
    ...
    try {
      const _key=typeof localStorage!=='undefined'?localStorage.getItem('onyx_api_key'):'';
      if(!_key){setStep(1);alert('Clé API manquante — ouvrez les paramètres ⚙ dans la sidebar.');return;}
        ...
        <div style={{padding:"24px 22px 20px"}}>
          ...
          {step===2&&(<>
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <button type="button" onClick={runAI} style={{flex:2,padding:10,borderRadius:7,border:"1px solid #b026ff30",background:"rgba(176,38,255,0.06)",color:"#b026ff",fontSize:9,letterSpacing:2,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>{IC.sparkle} GÉNÉRER MA VEILLE</button>
            </div>
          </>)}
...
function AddSourceModal({onClose,folders,onAddSource}) {
  ...
  const scan=async()=>{
    if(!url.trim())return;setScan(true);setSr(null);
    const clean=url.startsWith('http')?url:'https://'+url;
    const domain=clean.replace(/https?:\/\/(www\.)?/,'').split('/')[0];
    const candidates=[clean,`${clean.replace(/\/$/,'')}/feed`,`${clean.replace(/\/$/,'')}/rss`,`https://${domain}/feed`,`https://${domain}/rss.xml`,`https://${domain}/atom.xml`];
    let found=false;
    for(const c of candidates){
      try{
        const r=await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(c)}&count=3`);
        const d=await r.json();
        if(d.status==='ok'&&d.items?.length>0){setSr({rss:true,name:d.feed?.title||domain,url:c,n:d.items.length});found=true;break;}
      }catch{}
    }
    if(!found)setSr({rss:false,name:domain,url:clean,n:0});
    setScan(false);
  };
  const add=()=>{
        ...
        <div style={{padding:"24px 22px 20px"}}>
          ...
          {sr&&(<>
            <div style={{padding:10,borderRadius:6,marginBottom:10,background:sr.rss?"rgba(0,255,135,0.03)":"rgba(0,240,255,0.03)",border:`1px solid ${sr.rss?"#00ff8715":"#00f0ff15"}`}}>
              ...
              <div style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:sr.rss?"#00ff87":"#00f0ff"}}>{sr.rss?`RSS — ~${sr.n} articles`:`SCRAPING — ${sr.n} pages`}</div>
            </div>
...
export default function ONYXRadar({ onBack }){
  ...
  const[apiKey,setApiKey]=useState(()=>{try{return localStorage.getItem('onyx_api_key')||'';}catch{return '';}});
  ...
  const exportArchive=(mode="current")=>{
    const toExport=mode==="all"?articles:(filter.t==="static"?articles.filter(a=>a.isStatic):articles);
    const activeEngines=engines.filter(e=>e.active);
    const headers=["Titre","Source","Dossier(s)","Résumé","Date","URL",...activeEngines.map(e=>e.name),"Analyse IA","Notes"];
    const rows=toExport.map(a=>{
      const fnames=a.sf.map(fid=>srcF.find(f=>f.id===fid)?.name||'').filter(Boolean).join('; ');
      const tags=activeEngines.map(e=>(a.an[e.id]||[]).join('; '));
      return[a.title,a.sn,fnames,a.sum,a.date,a.url||'',...tags,a.ai||'',(a.notes||[]).join(' | ')];
    });
    const BOM='\uFEFF';
    const csv=[headers,...rows].map(r=>r.map(c=>`"${String(c||'').replace(/"/g,'""')}"`).join(',')).join('\n');
    const blob=new Blob([BOM+csv],{type:'text/csv;charset=utf-8'});
    const link=document.createElement('a');link.href=URL.createObjectURL(blob);
    link.download=`ONYX_export_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const AutoModal=()=>{
    ...
    const go=async()=>{
      if(!apiKey){alert('Clé API manquante — ouvrez les paramètres ⚙ dans la sidebar.');return;}
      setRun(true);
      const selEngData=engines.filter(e=>selE.includes(e.id));
      const results=[];
      for(const art of sa){
        const engPr=selEngData.map(en=>{
          if(en.extractRaw)return `"${en.id}" (${en.name}): extraire la valeur brute (chiffre, date…)`;
          const vals=en.values.filter(v=>v.on).map(v=>`"${v.v}"`);
          return `"${en.id}" (${en.name}): choisir parmi [${vals.join(', ')}]${en.name==='Sentiment'?' — UNE seule valeur':''}`;
        }).join('\n');
        const prompt=`Tu es un analyste de veille stratégique. Analyse cet article et attribue des tags.\n\nTitre: ${art.title}\nRésumé: ${art.sum}\n${art.content?`Contenu: ${art.content.slice(0,800)}`:''}\n\nANALYSEURS:\n${engPr}\n\nRéponds UNIQUEMENT en JSON valide, sans markdown ni explication:\n{${selEngData.map(en=>`"${en.id}":[]`).join(',')}}\n\nRègle: utilise UNIQUEMENT les valeurs listées. Tableau vide [] si rien ne correspond.`;
        try{
          const resp=await fetch('https://api.anthropic.com/v1/messages',{
            method:'POST',
            headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},
            body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:300,system:'Réponds UNIQUEMENT en JSON valide, sans markdown.',messages:[{role:'user',content:prompt}]})
          });
          const data=await resp.json();
          if(data.error)throw new Error(data.error.message);
          const raw=JSON.parse((data.content?.map(i=>i.text||'').join('')||'{}').replace(/```json|```/g,'').trim());
          const validated={};
          selEngData.forEach(en=>{
            const v=raw[en.id]||[];
            if(en.extractRaw){validated[en.id]=Array.isArray(v)?v:[String(v)];}
            else{const allowed=en.values.filter(x=>x.on).map(x=>x.v);validated[en.id]=(Array.isArray(v)?v:[v]).filter(x=>allowed.includes(x));}
          });
          results.push({aid:art.id,na:validated,sum:Object.values(validated).flat().join(' — ')});
        }catch(e){results.push({aid:art.id,na:{},sum:`Erreur: ${e.message?.slice(0,50)}`});}
      }
      setRes(results);setRun(false);setDone(true);
    };
    ...
    return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(14px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={()=>setShowAuto(false)}><div onClick={e=>e.stopPropagation()} style={{width:480,maxWidth:"94vw",maxHeight:"85vh",overflow:"auto",background:"#0b0b18",border:"1px solid #ffffff12",borderRadius:16,position:"relative"}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"linear-gradient(90deg, #b026ff, #00f0ff)",borderRadius:"16px 16px 0 0"}}/><div style={{padding:"22px 20px 18px"}}>
      ...
      {!done?(<>
        {hasEx&&<div style={{display:"flex",gap:5,marginBottom:12}}>{[{m:"merge",l:"Fusionner",c:"#00ff87"},{m:"overwrite",l:"Écraser",c:"#ff003c"}].map(o=>(<button key={o.m} type="button" onClick={()=>setMode(o.m)} style={{flex:1,padding:8,borderRadius:6,border:`1px solid ${mode===o.m?o.c+"30":"#ffffff06"}`,background:mode===o.m?`${o.c}08`:"transparent",cursor:"pointer",fontSize:10,fontWeight:600,color:mode===o.m?o.c:"#ffffff40",textAlign:"center"}}>{o.l}</button>))}</div>}
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:10}}>{IC.check}<span style={{fontSize:11,fontWeight:600,color:"#00ff87"}}>Terminé</span></div>
  ...
  const SettingsPanel=()=>{
    ...
    const save=()=>{
      try{localStorage.setItem('onyx_api_key',k);}catch{}
      setApiKey(k);setShowSettings(false);
    };
    const masked=k?k.slice(0,10)+'•••••••••'+k.slice(-4):'';
    return(
      <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.9)',backdropFilter:'blur(14px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}} onClick={()=>setShowSettings(false)}>
        <div onClick={e=>e.stopPropagation()} style={{width:440,maxWidth:'94vw',background:'#0b0b18',border:'1px solid #00f0ff15',borderRadius:16,overflow:'hidden',position:'relative'}}>
          <div style={{height:3,background:'linear-gradient(90deg,#00f0ff,#b026ff)'}}/>
          <div style={{padding:'24px 22px 20px'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:20}}>
              <span style={{fontSize:14}}>⚙</span>
              <span style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,fontWeight:700,letterSpacing:2,color:'#fff'}}>PARAMÈTRES ONYX</span>
              <button onClick={()=>setShowSettings(false)} style={{marginLeft:'auto',background:'none',border:'none',color:'#ffffff30',cursor:'pointer',fontSize:14,lineHeight:1}}>✕</button>
            </div>
            <div style={{marginBottom:14}}>
              <div style={{fontSize:8,letterSpacing:2,color:'#00f0ff50',fontFamily:"'JetBrains Mono',monospace",marginBottom:6}}>CLÉ API ANTHROPIC (CLAUDE)</div>
              <input type='password' value={k} onChange={e=>setK(e.target.value)} placeholder='sk-ant-api03-...'
                style={{width:'100%',padding:'10px 12px',boxSizing:'border-box',background:'rgba(255,255,255,0.02)',border:'1px solid #ffffff10',borderRadius:6,color:'#fff',fontSize:11,outline:'none',fontFamily:"'JetBrains Mono',monospace"}}/>
              <div style={{fontSize:8,color:'#ffffff15',marginTop:4,fontFamily:"'JetBrains Mono',monospace"}}>
                Obtenir sur console.anthropic.com · Stockée dans votre navigateur uniquement
              </div>
            </div>
            {apiKey&&(
              <div style={{padding:'7px 10px',borderRadius:5,background:'rgba(0,255,135,0.04)',border:'1px solid #00ff8718',marginBottom:14}}>
                <div style={{fontSize:8,color:'#00ff87',fontFamily:"'JetBrains Mono',monospace"}}>✓ Clé configurée : {masked}</div>
              </div>
            )}
            {!apiKey&&(
              <div style={{padding:'7px 10px',borderRadius:5,background:'rgba(255,0,60,0.04)',border:'1px solid #ff003c18',marginBottom:14}}>
                <div style={{fontSize:8,color:'#ff003c',fontFamily:"'JetBrains Mono',monospace"}}>⚠ Clé manquante — auto-tagging et configurateur IA désactivés</div>
              </div>
            )}
            ...
            <div style={{display:'flex',gap:6}}>
              <button onClick={()=>setShowSettings(false)} style={{flex:1,padding:10,borderRadius:6,border:'1px solid #ffffff06',background:'transparent',color:'#ffffff25',fontSize:9,letterSpacing:2,cursor:'pointer',fontFamily:"'JetBrains Mono',monospace"}}>ANNULER</button>
              <button onClick={save} style={{flex:2,padding:10,borderRadius:6,border:'1px solid #00f0ff25',background:'rgba(0,240,255,0.05)',color:'#00f0ff',fontSize:9,letterSpacing:2,cursor:'pointer',fontFamily:"'JetBrains Mono',monospace"}}>SAUVEGARDER</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{display:"flex",height:`${100/uiScale}vh`,width:`${100/uiScale}%`,transform:`scale(${uiScale})`,transformOrigin:"top left",background:"#07070d",fontFamily:"'Outfit',sans-serif",color:"#fff",overflow:"hidden"}}>
      ...
      <aside style={{width:col?44:242,minWidth:col?44:242,borderRight:"1px solid #ffffff05",background:"#090912",display:"flex",flexDirection:"column",transition:"0.25s",overflow:"hidden"}}>
        <div style={{padding:col?"10px 5px":"10px 10px",borderBottom:"1px solid #ffffff04",display:"flex",alignItems:"center",justifyContent:col?"center":"space-between"}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:2}}>
            {!col&&<button type="button" title="Actualiser les flux RSS" onClick={fetchFeeds} disabled={fetchingFeeds}
              style={{background:'none',border:'none',color:fetchingFeeds?'#00f0ff':'#ffffff18',cursor:'pointer',fontSize:12,lineHeight:1,transform:fetchingFeeds?'rotate(45deg)':'none',transition:'0.3s'}}>↺</button>}
            {!col&&<button type="button" title="Paramètres" onClick={()=>setShowSettings(true)}
              style={{background:'none',border:'none',color:apiKey?'#ffffff18':'#ff003c88',cursor:'pointer',fontSize:11,lineHeight:1}}>⚙</button>}
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <header style={{padding:"7px 14px",borderBottom:"1px solid #ffffff04",display:"flex",alignItems:"center",gap:6,background:"#09091280"}}>
          ...
          {/* Selection actions */}
          {selIds.length>0&&(<>
            <span style={{fontSize:8,color:"#00f0ff55",fontFamily:"'JetBrains Mono',monospace"}}>{selIds.length} sél.</span>
                ...
                {feed.map(art=>{
                  ...
                  return(<div key={art.id} style={{padding:"10px 11px",borderRadius:6,border:`1px solid ${isSel?"#00f0ff1a":"#ffffff03"}`,borderLeft:`3px solid ${art.sc}`,background:isSel?"rgba(0,240,255,0.015)":"rgba(255,255,255,0.004)",transition:"0.1s"}} onMouseEnter={e=>{if(!isSel)e.currentTarget.style.background="rgba(255,255,255,0.01)";}} onMouseLeave={e=>{e.currentTarget.style.background=isSel?"rgba(0,240,255,0.015)":"rgba(255,255,255,0.004)";}}>
                    <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:4}}>
                      ...
                      {!hasAn&&<span style={{fontSize:7,padding:"1px 4px",borderRadius:2,background:"#ffffff04",color:"#ffffff15",fontFamily:"'JetBrains Mono',monospace"}}>non analysé</span>}
